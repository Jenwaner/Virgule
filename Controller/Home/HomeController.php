...
class HomeController extends AbstractController
{
     ...
     * @Route("/", name="home")
     */
    public function index(ProductRepository $productRepository, SettingRepository $settingRepository, Request $request, PaginatorInterface $paginator)
    {
        $slider = $productRepository->findBy([], ['title' => 'ASC'], 6);
        $allproduct = $productRepository->findBy([], ['title' => 'ASC']);
        $newproduct = $productRepository->findBy([], ['title' => 'ASC'], 5);
        $data = $settingRepository->findBy(['id' => 1]);

        $products = $paginator ->paginate(
            $allproduct, // Je passe les données
            $request->query->getInt('page', 1),9  //numero de la page en cours, 1 par default
        );
